            ...
            <ScrollViewer
                ...
                <Border
                    ...
                    <StackPanel>
                        ...
                        <Expander
                            Header="⚙️ Infrastructure Services"
                            IsExpanded="True"
                            Style="{StaticResource ExpanderStyle}">
                            <StackPanel>
                                <Button
                                    x:Name="ButtonDhcpReport"
                                    Content="DHCP Server Report"
                                    Style="{StaticResource SidebarButton}"
                                    ToolTip="Generates a report of DHCP scopes and statistics." />
                                <Button
                                    x:Name="ButtonDnsReport"
                                    Content="DNS Server Report"
                                    Style="{StaticResource SidebarButton}"
                                    ToolTip="Generates a report of DNS zones." />
                                <Button
                                    x:Name="ButtonIisReport"
                                    Content="IIS Server Report"
                                    Style="{StaticResource SidebarButton}"
                                    ToolTip="Generates a report of IIS websites and application pools." />
                                <Button
                                    x:Name="ButtonWsusReport"
                                    Content="WSUS Server Report"
                                    Style="{StaticResource SidebarButton}"
                                    ToolTip="Generates a summary report of the WSUS server." />
                                <Button
                                    x:Name="ButtonRdsFarmReport"
                                    Content="RDS Farm Report"
                                    Style="{StaticResource SidebarButton}"
                                    ToolTip="Generates a report of RDS collections and session hosts." />
                                <Button
                                    x:Name="ButtonRdsServerReport"
                                    Content="RDS Server Report"
                                    Style="{StaticResource SidebarButton}"
                                    ToolTip="Generates a report of published RemoteApps on a session host." />
                                <Button
                                    x:Name="ButtonDfsReport"
                                    Content="DFS Namespace Report"
                                    Style="{StaticResource SidebarButton}"
                                    ToolTip="Generates a report of DFS namespaces and their configuration." />
                            </StackPanel>
                        </Expander>
        ...
        <Border
            ...
            <Grid Margin="20,0">
                ...
                <TextBlock
                    ...
                    FontSize="11"
                    Foreground="#E5E7EB"
                    Text="easyADReport  |  Version: v0.6.1  |  Last Update: 25.10.2025" />
...
Function Get-PrivilegedAccounts {
    ...
    try {
        ...
        $UniquePrivilegedAccounts = $AllPrivilegedAccounts | Sort-Object SamAccountName -Unique | ForEach-Object {
            ...
            # Risikostufe wird später im Objekt bestimmt
...
Function Get-ASREPRoastableAccounts {
    ...
    try {
        ...
        foreach ($user in $ASREPUsers) {
            ...
            # Risiko-Level wird direkt im PSCustomObject bestimmt
...
#--- Universelle Funktion zur Ausführung von Infrastruktur-Berichten ---
#--- DHCP-Berichtsfunktion ---
#--- DNS-Berichtsfunktion ---
#--- IIS-Berichtsfunktion ---
#--- WSUS-Berichtsfunktion ---
#--- RDS Farm-Berichtsfunktion ---
#--- RDS Server-Berichtsfunktion ---
#--- DFS-Berichtsfunktion ---
Function Get-DfsReport {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Sammle DFS-Namespace-Informationen..." -Type Info
    try {
        # Importiere das DFSN-Modul
        ...
        if (-not $roots) {
            Write-ADReportLog -Message "Keine DFS-Namespaces auf diesem Server gefunden." -Type Warning
            return @()
        }

        $reportData = foreach ($root in $roots) {
            $targets = Get-DfsnRootTarget -Path $root.Path
            $folders = Get-DfsnFolder -Path "$($root.Path)\*"
        ...
        Write-ADReportLog -Message "DFS-Bericht mit $($reportData.Count) Namespaces erfolgreich erstellt." -Type Success
        return $reportData
    } catch {
        $errorMessage = "Fehler beim Abrufen des DFS-Berichts: $($_.Exception.Message)"
        Write-ADReportLog -Message $errorMessage -Type Error
        return @()
    }
}

Function Get-RdsServerReport {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Sammle RDS Server-Informationen (RemoteApps)..." -Type Info
    try {
        # Importiere das RemoteDesktopServices-Modul
        if (-not (Get-Module -Name RemoteDesktopServices -ErrorAction SilentlyContinue)) {
            Import-Module RemoteDesktopServices -ErrorAction Stop
        }

        # Dieser Befehl muss auf einem Sitzungshost ausgeführt werden, um die lokalen RemoteApps zu erhalten.
        # Für Remote-Abfragen wird der Verbindungsbroker als Parameter benötigt, was die Komplexität erhöht.
        # Fürs Erste konzentrieren wir uns auf die RemoteApps, die vom Broker für Sammlungen veröffentlicht wurden.
        $remoteApps = Get-RDRemoteApp -ErrorAction Stop

        if (-not $remoteApps) {
            Write-ADReportLog -Message "Keine RemoteApps auf diesem Server gefunden." -Type Warning
            return @()
        }

        $reportData = foreach ($app in $remoteApps) {
            [PSCustomObject]@{
        ...
        Write-ADReportLog -Message "RDS Server-Bericht mit $($reportData.Count) RemoteApps erfolgreich erstellt." -Type Success
        return $reportData
    } catch {
        $errorMessage = "Fehler beim Abrufen des RDS Server-Berichts: $($_.Exception.Message)"
        Write-ADReportLog -Message $errorMessage -Type Error
        return @()
    }
}

Function Get-RdsFarmReport {
    [CmdletBinding()]
    param()

    Write-ADReportLog -Message "Sammle RDS Farm-Informationen..." -Type Info
    try {
        # Importiere das RemoteDesktopServices-Modul
        if (-not (Get-Module -Name RemoteDesktopServices -ErrorAction SilentlyContinue)) {
            Import-Module RemoteDesktopServices -ErrorAction Stop
        ...
        $collections = Get-RDCollection -ErrorAction Stop
        if (-not $collections) {
            Write-ADReportLog -Message "Keine RDS-Sammlungen auf diesem Server (Verbindungsbroker) gefunden." -Type Warning
            return @()
        }

        $reportData = foreach ($collection in $collections) {
            $sessionHosts = Get-RDSessionHost -CollectionName $collection.CollectionName
            $userSessions = Get-RDUserSession -CollectionName $collection.CollectionName
            
            [PSCustomObject]@{
                CollectionName = $collection.CollectionName
                Description = $collection.CollectionDescription
                SessionHostServers = ($sessionHosts.SessionHost -join ", ")
                NumberOfSessionHosts = $sessionHosts.Count
                ActiveSessions = ($userSessions | Where-Object { $_.SessionState -eq 'Active' }).Count
                DisconnectedSessions = ($userSessions | Where-Object { $_.SessionState -eq 'Disconnected' }).Count
                TotalSessions = $userSessions.Count
            }
        }
        
        Write-ADReportLog -Message "RDS Farm-Bericht mit $($reportData.Count) Sammlungen erfolgreich erstellt." -Type Success
        return $reportData
    } catch {
        $errorMessage = "Fehler beim Abrufen des RDS Farm-Berichts: $($_.Exception.Message)"
        Write-ADReportLog -Message $errorMessage -Type Error
        return @()
    }
}
...
Function Get-WsusReport {
    ...
    Write-ADReportLog -Message "Sammle WSUS-Informationen..." -Type Info
    try {
        # Importiere das UpdateServices-Modul, falls noch nicht geladen
        if (-not (Get-Module -Name UpdateServices -ErrorAction SilentlyContinue)) {
            Import-Module UpdateServices -ErrorAction Stop
        }

        $wsus = Get-WsusServer -ErrorAction Stop
        if (-not $wsus) {
            Write-ADReportLog -Message "Keine WSUS-Instanz auf diesem Server gefunden." -Type Warning
            return @()
        }

        $computers = Get-WsusComputer -All
        $updates = Get-WsusUpdate -Approval Any -Status Any

        $reportData = [PSCustomObject]@{
            ServerVersion = $wsus.Version
            TotalComputers = $computers.Count
            ComputersNeedingUpdates = ($computers | Where-Object { $_.LastStatusReport -lt (Get-Date).AddDays(-7) -or $_.UpdatesNeedingInstallation -gt 0 }).Count
            TotalUpdates = $updates.Count
            ApprovedUpdates = ($updates | Where-Object { $_.IsApproved }).Count
            NotApprovedUpdates = ($updates | Where-Object { -not $_.IsApproved }).Count
            LastSyncTime = $wsus.GetSubscription().GetLastSynchronizationTime()
            LastSyncResult = $wsus.GetSubscription().GetLastSynchronizationResult()
        }
        
        Write-ADReportLog -Message "WSUS-Bericht erfolgreich erstellt." -Type Success
        return $reportData
    } catch {
        $errorMessage = "Fehler beim Abrufen des WSUS-Berichts: $($_.Exception.Message)"
        Write-ADReportLog -Message $errorMessage -Type Error
        return @()
    }
}
...
Function Get-IisReport {
    ...
    Write-ADReportLog -Message "Sammle IIS-Informationen..." -Type Info
    try {
        # Importiere das WebAdministration-Modul, falls noch nicht geladen
        ...
        if (-not $sites) {
            Write-ADReportLog -Message "Keine IIS-Websites auf diesem Server gefunden." -Type Warning
            return @()
        }

        $reportData = foreach ($site in $sites) {
            $appPools = Get-WebAppPoolState -Name $site.Applications["/"].ApplicationPoolName
            [PSCustomObject]@{
                Name = $site.Name
                ID = $site.Id
                State = $site.State
                ...
                AppPoolState = if($appPools) { $appPools.IsStarted } else { 'Unknown' }
                Bindings = ($site.bindings.collection | ForEach-Object { $_.bindingInformation }) -join ", "
            }
        }
        Write-ADReportLog -Message "IIS-Bericht mit $($reportData.Count) Websites erfolgreich erstellt." -Type Success
        return $reportData
    } catch {
        $errorMessage = "Fehler beim Abrufen des IIS-Berichts: $($_.Exception.Message)"
        Write-ADReportLog -Message $errorMessage -Type Error
        return @()
    }
}
...
Function Get-DnsReport {
    ...
    Write-ADReportLog -Message "Sammle DNS-Server-Informationen..." -Type Info
    try {
        # Importiere das DNS-Server-Modul, falls noch nicht geladen
        if (-not (Get-Module -Name DnsServer -ErrorAction SilentlyContinue)) {
            Import-Module DnsServer -ErrorAction Stop
        }

        $zones = Get-DnsServerZone -ErrorAction Stop
        if (-not $zones) {
            Write-ADReportLog -Message "Keine DNS-Zonen auf diesem Server gefunden." -Type Warning
            return @()
        }

        $reportData = foreach ($zone in $zones) {
            [PSCustomObject]@{
                ZoneName = $zone.ZoneName
                ZoneType = $zone.ZoneType
        ...
        Write-ADReportLog -Message "DNS-Bericht mit $($reportData.Count) Zonen erfolgreich erstellt." -Type Success
        return $reportData
    } catch {
        $errorMessage = "Fehler beim Abrufen des DNS-Berichts: $($_.Exception.Message)"
        Write-ADReportLog -Message $errorMessage -Type Error
        return @()
    }
}
...
Function Get-DhcpReport {
    ...
    Write-ADReportLog -Message "Sammle DHCP-Server-Informationen..." -Type Info
    try {
        # Importiere das DHCP-Server-Modul, falls noch nicht geladen
        if (-not (Get-Module -Name DHCPServer -ErrorAction SilentlyContinue)) {
            Import-Module DHCPServer -ErrorAction Stop
        }

        $scopes = Get-DhcpServerv4Scope -ErrorAction Stop
        if (-not $scopes) {
            Write-ADReportLog -Message "Keine DHCP-Bereiche auf diesem Server gefunden." -Type Warning
            return @()
        }

        $reportData = foreach ($scope in $scopes) {
            ...
            [PSCustomObject]@{
                ...
                PercentageUsed = if ($stats.TotalAddresses -gt 0) { "{0:P2}" -f ($stats.InUse / $stats.TotalAddresses) } else { "N/A" }
            }
        }
        Write-ADReportLog -Message "DHCP-Bericht mit $($reportData.Count) Bereichen erfolgreich erstellt." -Type Success
        return $reportData
    } catch {
        $errorMessage = "Fehler beim Abrufen des DHCP-Berichts: $($_.Exception.Message)"
        Write-ADReportLog -Message $errorMessage -Type Error
        # Gib eine leere Liste zurück, um UI-Fehler zu vermeiden
        return @()
    }
}
...
Function Invoke-ReportAction {
    param(
        [Parameter(Mandatory=$true)]
        [string]$FeatureName, # z.B. 'DHCP', 'DNS'

        [Parameter(Mandatory=$true)]
        [string]$WindowsFeatureName, # z.B. 'DHCP', 'DNS'

        [Parameter(Mandatory=$true)]
        [scriptblock]$ReportScriptBlock
    )

    try {
        # 1. Prüfen, ob die Rolle lokal installiert ist
        Write-ADReportLog -Message "Prüfe, ob die Rolle '$WindowsFeatureName' lokal installiert ist..." -Type Info
        $localFeature = Get-WindowsFeature -Name $WindowsFeatureName -ErrorAction SilentlyContinue

        if ($localFeature -and $localFeature.Installed) {
            Write-ADReportLog -Message "'$WindowsFeatureName' ist lokal installiert. Führe Bericht lokal aus." -Type Info
            $results = & $ReportScriptBlock
            return $results
        }

        # 2. Wenn nicht lokal, frage nach Remote-Server
        Write-ADReportLog -Message "'$WindowsFeatureName' ist nicht lokal installiert. Fordere Remote-Server an." -Type Info
        
        # PS2EXE-konformer Input-Dialog für den Servernamen
        Add-Type -AssemblyName Microsoft.VisualBasic
        $serverName = [Microsoft.VisualBasic.Interaction]::InputBox("Die Rolle '$FeatureName' ist auf diesem Server nicht installiert. Bitte geben Sie den Namen des Zielservers an, auf dem der Bericht ausgeführt werden soll.", "Remote-Server für $FeatureName-Bericht", "")

        if ([string]::IsNullOrWhiteSpace($serverName)) {
            Write-ADReportLog -Message "Kein Remote-Server angegeben. Vorgang abgebrochen." -Type Warning
            return $null
        }

        # 3. Anmeldeinformationen sicher abfragen
        Write-ADReportLog -Message "Frage Anmeldeinformationen für '$serverName' ab..." -Type Info
        $credential = Get-Credential -UserName "$env:USERDOMAIN\$env:USERNAME" -Message "Bitte geben Sie die Anmeldeinformationen für '$serverName' ein."

        if (-not $credential) {
            Write-ADReportLog -Message "Keine Anmeldeinformationen angegeben. Vorgang abgebrochen." -Type Warning
            return $null
        }

        # 4. Bericht remote ausführen
        Write-ADReportLog -Message "Führe '$FeatureName'-Bericht auf '$serverName' remote aus..." -Type Info
        
        $remoteResults = Invoke-Command -ComputerName $serverName -Credential $credential -ScriptBlock $ReportScriptBlock -ErrorAction Stop
        
        Write-ADReportLog -Message "Remote-Bericht erfolgreich ausgeführt." -Type Success
        return $remoteResults

    } catch {
        $errorMessage = "Fehler in Invoke-ReportAction für '$FeatureName': $($_.Exception.Message)"
        Write-ADReportLog -Message $errorMessage -Type Error
        [System.Windows.MessageBox]::Show($errorMessage, "Fehler bei der Berichtsausführung", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        return $null
    }
}
    ...
    # Infrastructure Services Buttons
    $Global:ButtonDhcpReport = $Window.FindName("ButtonDhcpReport")
    $Global:ButtonDnsReport = $Window.FindName("ButtonDnsReport")
    $Global:ButtonIisReport = $Window.FindName("ButtonIisReport")
    $Global:ButtonWsusReport = $Window.FindName("ButtonWsusReport")
    $Global:ButtonRdsFarmReport = $Window.FindName("ButtonRdsFarmReport")
    $Global:ButtonRdsServerReport = $Window.FindName("ButtonRdsServerReport")
    $Global:ButtonDfsReport = $Window.FindName("ButtonDfsReport")
...
"@
    ...
    # Event Handler für DFS Report Button
    if ($null -ne $Global:ButtonDfsReport) {
        $ButtonDfsReport.add_Click({
            Write-ADReportLog -Message "Starte DFS-Namespace-Bericht..." -Type Info
            $reportData = Invoke-ReportAction -FeatureName "DFS" -WindowsFeatureName "FS-DFS-Namespace" -ReportScriptBlock { . ([scriptblock]::Create((Get-Content -Path $MyInvocation.MyCommand.Definition -Raw) + ";Get-DfsReport")) }
    ...
    # Event Handler für RDS Server Report Button
    if ($null -ne $Global:ButtonRdsServerReport) {
        $ButtonRdsServerReport.add_Click({
            Write-ADReportLog -Message "Starte RDS Server-Bericht (RemoteApps)..." -Type Info
            $reportData = Invoke-ReportAction -FeatureName "RDS Server" -WindowsFeatureName "RDS-RD-Server" -ReportScriptBlock { . ([scriptblock]::Create((Get-Content -Path $MyInvocation.MyCommand.Definition -Raw) + ";Get-RdsServerReport")) }
            if ($null -ne $reportData) {
                Update-ADReportResults -Results $reportData
            }
        })
    ...
    # Event Handler für RDS Farm Report Button
    if ($null -ne $Global:ButtonRdsFarmReport) {
        $ButtonRdsFarmReport.add_Click({
            Write-ADReportLog -Message "Starte RDS Farm-Bericht..." -Type Info
            $reportData = Invoke-ReportAction -FeatureName "RDS Farm" -WindowsFeatureName "RDS-Connection-Broker" -ReportScriptBlock { . ([scriptblock]::Create((Get-Content -Path $MyInvocation.MyCommand.Definition -Raw) + ";Get-RdsFarmReport")) }
            if ($null -ne $reportData) {
                Update-ADReportResults -Results $reportData
            }
        })
    ...
    # Event Handler für WSUS Report Button
    if ($null -ne $Global:ButtonWsusReport) {
        $ButtonWsusReport.add_Click({
            Write-ADReportLog -Message "Starte WSUS-Server-Bericht..." -Type Info
            $reportData = Invoke-ReportAction -FeatureName "WSUS" -WindowsFeatureName "UpdateServices" -ReportScriptBlock { . ([scriptblock]::Create((Get-Content -Path $MyInvocation.MyCommand.Definition -Raw) + ";Get-WsusReport")) }
            if ($null -ne $reportData) {
                Update-ADReportResults -Results @($reportData)
            }
        })
    ...
    # Event Handler für IIS Report Button
    if ($null -ne $Global:ButtonIisReport) {
        $ButtonIisReport.add_Click({
            Write-ADReportLog -Message "Starte IIS-Server-Bericht..." -Type Info
            $reportData = Invoke-ReportAction -FeatureName "IIS" -WindowsFeatureName "Web-Server" -ReportScriptBlock { . ([scriptblock]::Create((Get-Content -Path $MyInvocation.MyCommand.Definition -Raw) + ";Get-IisReport")) }
            if ($null -ne $reportData) {
                Update-ADReportResults -Results $reportData
            }
        })
    ...
    # Event Handler für DNS Report Button
    if ($null -ne $Global:ButtonDnsReport) {
        $ButtonDnsReport.add_Click({
            Write-ADReportLog -Message "Starte DNS-Server-Bericht..." -Type Info
            $reportData = Invoke-ReportAction -FeatureName "DNS" -WindowsFeatureName "DNS" -ReportScriptBlock { . ([scriptblock]::Create((Get-Content -Path $MyInvocation.MyCommand.Definition -Raw) + ";Get-DnsReport")) }
            if ($null -ne $reportData) {
                Update-ADReportResults -Results $reportData
            }
        })
    ...
    # Event Handler für DHCP Report Button
    if ($null -ne $Global:ButtonDhcpReport) {
        $ButtonDhcpReport.add_Click({
            Write-ADReportLog -Message "Starte DHCP-Server-Bericht..." -Type Info
            $reportData = Invoke-ReportAction -FeatureName "DHCP" -WindowsFeatureName "DHCP" -ReportScriptBlock { . ([scriptblock]::Create((Get-Content -Path $MyInvocation.MyCommand.Definition -Raw) + ";Get-DhcpReport")) } 
            if ($null -ne $reportData) {
                Update-ADReportResults -Results $reportData
            }
        })
...
# MjUxMDI1MTcxOTA5WjA3BgsqhkiG9w0BCRACLzEoMCYwJDAiBCDPodw1ne0rw8uJ
...
# tKcCbUmMbHiNqjANBgkqhkiG9w0BAQEFAASCAgCptkxOAGjS+FVs7yMD9q58MsHf
# CtDknvue04auzNGGZKRHxfS8maPWzbTrYtSvXQcVHX0W7ULT39Nxcv9aF4bYs9aE
# BPs+/oFjE7KUx9YUHVFLyS2qsxzwwYQrYQFiYJ8rnCExoDfUN1O5GCgi5Mhv+YM9
# 8DSlfR4hzWA8QDlBg2r4KjSi0hhcfnAYOEcrU5kWdS1F6biiszdcdeaECeY0iHGt
# BXkRttjU6HFWUzIpyEitKvtprP2R1TevuMQjA7sKKB2RNnIAYbtewhXhqMkIxtN5
# R9SVB+TCsfKUNund/ugKjKrj8imaIFbiIC/5c1dZSOpRybGnoQjcuxj1pKNsWPIH
# UP4peZlCEOMMmFLdQvnrd8xCggGjQpVbv+Iqxd0647/XgSTcI1MwUpxGAcZymXB3
# Z5V6ZpuCbshlOJH6uJw/agDpMJ48CJbIaE3c4MXYBqER+P7ujQExqN1wlVwMqmfH
# TVlbru0wbGW6mYVRP21Ipn7GKg6HXbiRbq/iATaCgH/MMSz1DXEaafDe39582q7u
# aoWQHQeiM13Fo18Siu65HKf2wWP1pMnd538gwdvDazXLu3JQwbN7NtHvDM0e/wMh
# Q4p8LhZLiVEAgNn92on3clF9UtbcekIhlx1bxSlvgqoYJ0clEwxkMS0qp/+NRhlx
# OuSgucXm3ZpBfxenYQ==
